...
public class FormMain : Form
{
	public class KeyParam
	{
		...
		public static byte ReportID = 0;	//0 is default report id, 1 is report id for multimedia keys, 2 is report id for mouse keys

		public static byte KEY_Cur_Layer = 1;

		public static byte KEY_Cur_Page = 1;  //1 key 2 Ctrl Shift Alt 3 Multimedia 4 LED 5 Mouse

		public static byte Language_Set = 0;  //0 English 1 Chinese
	}

	//Value 1 use HidLib.cs , Value 0 use Hid.cs
	private ushort WriteMode = 1;	//Default 1. Didn't appear other value.
	...
	//The keys on mini keyboard from 1 to 16 and spinners from 1 to 3
	private Button KEY1;
	...
	private void Show_Dowload_Text()
	{
		...
		label_Dowload_Dsp.Invoke((MethodInvoker)delegate
		{
    ...
    private void Timer1_Elapsed(object sender, ElapsedEventArgs e)
    {
        ...
        if (KeyParam.Data_Send_Buff[KeyParam.KeySet_KeyNum] != 0)
        {
            ...
            SetText.Invoke((MethodInvoker)delegate {
                SetText.Text = text;
            });
            ...
            SetFunText.Invoke((MethodInvoker)delegate {
	...
	//Check the keyboard version and set the report id
	//The keyboard version in my hand is 3
        ...
        if (WriteMode == 0)
        {
            ...
            if ((byte)myHid.Write(r) == 0)
            {
                label_Dowload_Dsp.Invoke((MethodInvoker)delegate {
                    ...
                    label_Dowload_Dsp.BackColor = Color.Green;
                    Show_Dowload_Text();
                });
            }
            else
            {
                label_Dowload_Dsp.Invoke((MethodInvoker)delegate {
                    ...
                    label_Dowload_Dsp.BackColor = Color.Red;
                    Show_Dowload_Text();
                });
            }
        }
    ...
    private void Send_WriteFlashLED_Cmd()
    {
        byte[] array = new byte[65];
        array[0] = 170;
        array[1] = 161;	//161 change led mode, didn't know if the led mode can be added freely
        if (WriteMode == 0)//Maybe the old write mode
        {
            report r = new report(KeyParam.ReportID, array);
            if ((byte)myHid.Write(r) == 0)
            {
        ...
        {
            if (WriteMode != 1)//Maybe in case of unexpected change of write mode
            {
                return;
            }
            if (myHidLib.WriteDevice(KeyParam.ReportID, array))
    ...
    private void Send_SwLayer()//Didn't know the function of the layer
	{
		byte[] array = new byte[65];
		array[0] = 161;
	...
	private void Download_Click(object sender, EventArgs e)//Flash
	{
		byte b = 0;
		byte[] array = new byte[65];
		if (!myHidLib.Get_Dev_Sta())
